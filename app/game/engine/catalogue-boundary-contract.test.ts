...
const RETIRED_ITEM_DEFINITION_IDS = ["food", "medicine", "rope"] as const;
...
describe("catalogue boundaries", () => {
  it("keeps generic food and medicine as capabilities rather than item definitions", () => {
    const itemDefinitionIds = new Set<string>(ITEM_CATALOGUE.map((definition) => definition.id));

    expect(itemDefinitionIds.has("food")).toBe(false);

    expect(itemDefinitionIds.has("medicine")).toBe(false);

    /*
     * These remain valid item tags describing
     * concrete food and medical items.
     */
    expect(ITEM_TAGS).toEqual(expect.arrayContaining(["food", "medicine"]));
  });
  ...
  it("keeps retired items out of events and acquisition pools", () => {
  ...
  it("keeps ordinary defense out of contextual item capabilities", () => {
