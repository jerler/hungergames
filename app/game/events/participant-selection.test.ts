...
const REST_TARGETING_NIGHT = {
  day: 2,
  period: "night",
} as const;

function withRestForTargeting(tribute: GameTribute, quality: NightRestQuality): GameTribute {
  return {
    ...tribute,
...
function createRestTargetingDefinition({
  targeting,
  environmental,
}: {
  targeting?: "hostile";
  ...
  const tags: EventDefinition["tags"] = environmental ? ["hazard", "environment"] : ["hazard"];

  return {
    id: "rest-targeting-test",
...
function createRestTargetingFixture() {
  const originalState = createTestGame("rest-targeting");

  const sheltered = withRestForTargeting(originalState.tributes[0], "sheltered");

  const unsheltered = withRestForTargeting(originalState.tributes[1], "unsheltered");

  const state: GameState = {
    ...originalState,

    tributes: originalState.tributes.map((tribute) => {
      if (tribute.id === sheltered.id) {
        return sheltered;
      }

      if (tribute.id === unsheltered.id) {
        return unsheltered;
      }
...
describe("night-rest participant targeting", () => {
  it("weights an unsheltered tribute more heavily for hostile roles", () => {
    const { state, sheltered, unsheltered } = createRestTargetingFixture();

    const selection = selectEventParticipants(
      createRestTargetingDefinition({
        targeting: "hostile",
...

      {
        ...
        livingTributes: [sheltered, unsheltered],
      },

      /*
       * Equal weights would select the first
       * tribute at exactly 0.5.
       *
       * The 1.5 unsheltered multiplier moves
       * this threshold into the second tribute.
       */
      () => 0.5,
    ...
    expect(selection?.participantsByRole.target?.[0]?.id).toBe(unsheltered.id);
  });

  it("weights an unsheltered tribute more heavily for environmental night hazards", () => {
    const { state, sheltered, unsheltered } = createRestTargetingFixture();
